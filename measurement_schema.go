...
func (m *MeasurementSchema) validateName(prefix, name string) error {
	if len(name) == 0 {
		return fmt.Errorf("%s %q: %w", prefix, name, ErrMeasurementSchemaNameTooShort)
	}

	if len(name) > 128 {
		return fmt.Errorf("%s %q: %w", prefix, name, ErrMeasurementSchemaNameTooLong)
	}

	if err := models.CheckToken([]byte(name)); err != nil {
		return fmt.Errorf("%s %q: %w", prefix, name, &influxerror.Error{
	...
	if strings.HasPrefix(name, "_") {
		return fmt.Errorf("%s %q: %w", prefix, name, ErrMeasurementSchemaNameUnderscore)
	}

	if strings.Contains(name, `"`) || strings.Contains(name, `'`) {
		return fmt.Errorf("%s %q: %w", prefix, name, ErrMeasurementSchemaNameQuotes)
	}
